...
// normalizeAndRead reads a .tgz and repacks it deterministically (NormalizeTgz).
func normalizeAndRead(tgzPath string) ([]byte, error) {
	origData, err := os.ReadFile(tgzPath) //nolint:gosec // build-produced artifact path from the packaging pipeline
	if err != nil {
		return nil, fmt.Errorf("read tgz: %w", err)
	}

	return NormalizeTgz(origData)
}
